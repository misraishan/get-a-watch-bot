...
import { ChatInputCommandInteraction, CacheType, time as djsTime } from "discord.js";
import { db } from "../..";
...
export async function setReminder(interaction: ChatInputCommandInteraction<CacheType>) {
    const currentDate = new Date();
    const time = interaction.options.get("time")?.value as string;
    const date = interaction.options.get("date")?.value as number;
    const month = interaction.options.get("month")?.value ? interaction.options.get("month")?.value as number : currentDate.getMonth() + 1;
    const year = interaction.options.get("year")?.value ? interaction.options.get("year")?.value as number : currentDate.getFullYear();
    ...
    if (!timezone) {
        ...
        if (!user) {
            await interaction.reply({content: "Please provide a timezone or update your own with /timezone.", ephemeral: true});
            return;
        }
    ...
    if (userTimers.length >= 10) {
        await interaction.reply({content: "You can only have 10 active reminders at a time!", ephemeral: true});
        return;
    }
    ...
    try {
        ...
        await interaction.reply({
            content: `Reminder set for ${formattedTimestamp}! It'll arrive in ${djsTime(timestamp / 1000, "R")}.
                \nMake sure you allow DMs.`, 
            ephemeral: true
        });
    } catch (error) {
        console.warn(error);
        await interaction.reply({content: "An error occurred while setting your Reminder!", ephemeral: true});
    }
}